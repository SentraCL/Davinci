<template>
  <div>
    <span v-if="!inactive">
      <div class="row">
        <div class="col-12">
          <textarea class="ta8" placeholder="Ingrese información del paso (Si presiona Shift + Enter se agregara automaticamente)" id="txtPaso" v-model="step.step" @keydown.enter.shift.exact.prevent="addStep()"></textarea>
        </div>
      </div>
      <div class="row">
        <div class="col-12">
      ...
      <div class="row">
        <div class="col-12">
          <textarea class="ta8" placeholder="Resultado del Paso" id="txtResultado" v-model="step.result" @keydown.enter.shift.exact.prevent="addStep()"></textarea>
        </div>
      </div>
      <div class="row">
        <div class="col-10">
        ...
        <div class="col-2">
          <button type="button" @click="addStep()" style="width: 100%" class="btn sub-button"><i class="ti-notepad"></i> Agregar</button>
        </div>
      </div>
      <br />
      <br />
    ...
    <div class="scrollingSteps" v-if="localSteps.length>0">
      <table class="table scroll" id="tblPasos">
        <tbody style="background-color: #fff;">
          <tr height="32px">
            <td width="5%"><strong>Orden</strong></td>
            <!--<th>ID</th>-->
            <td width="40%"><strong>Descripcion del Paso</strong></td>
            <td width="40%"><strong>Resultado Esperado</strong></td>
            <td width="10%" class="lastItem" v-if="!inactive"><strong>Opción</strong></td>
          </tr>

          <tr v-for="item,step in localSteps" height="32px">
            <td width="5%" class="stepCol">{{step+1}}</td>
            <!-- <th width="5%" class="stepCol">P-{{item.id}}</th> -->
            <td width="40%"><small :title="item.step">{{item.step | readMore(140, '...') }}</small></td>
            <td width="45%"><small :title="item.result">{{item.result | readMore(140, '...')}}</small></td>
            <td width="5%" class="lastItem" v-if="!inactive">
              <span class="up">
                <span class="ti-arrow-up" title="Subir Paso" v-on:click="up(item.id)"></span></span>
              <span class="down">
                <span class="ti-arrow-down" title="Bajar Paso" v-on:click="down(item.id)"></span></span>
              <span class="edit">
                <span class="ti-pencil-alt" title="Editar Paso" v-on:click="edit(item.id)"></span></span>
              <span class="del">
                <span class="ti-trash" title="Eliminar Paso" v-on:click="deleteStep(item.id)"></span></span>
            </td>
          </tr>
  ...
  export default {
    ...
    methods: {
      ...
      edit(id) {
        ...
        this.editDialog.show = false;
        this.localSteps[this.editDialog.step.id - 1] = this.cloneObject(this.editDialog.step);
