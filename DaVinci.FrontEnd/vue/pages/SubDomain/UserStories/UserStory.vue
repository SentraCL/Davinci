    ...
    <template slot="header">
      <div class="topnav">
        <div class="user-stories-title" :name="userStoryCode">
          <span class="us-title"> <span v-if="userStoryCode==''" v-html="userStoryForm.title"></span> {{userStoryCode}} </span>
          <small class="us-ver">
            <combo-simple v-if="isUserStoryExist && versions.length>0" v-on:change="changeVersion()" :showKey="true" label="" class="input-item" :list="versions" keyValue="version" keyLabel="ref" :value.sync="currentVer"></combo-simple>
          </small>
          <a class="btn btn-xs us-btn" title="Cerrar" @click="remove"><i class="ti-close"></i></a>
          <a class="btn btn-xs us-btn" v-if="currentVer!=lastVersion" title="No es posible Editar"><i class="ti-lock"></i></a>
          <a class="btn btn-xs us-btn" v-if="isUserStoryExist && currentVer==lastVersion && versions.length>0" title="Guardar" @click="save"><i class="ti-save"></i></a>
          <a class="btn btn-xs us-btn" v-if="currentVer==lastVersion" title="Crear Nueva Version" @click="create"><i class="ti-wand"></i></a>
        </div>
      </div>
    ...
    <div style="padding-left:16px">
      <h-tabs :tabs="UsTabs" viewScoped v-if="showForm" v-on:loadTab="similarUserStories()">
        <span slot="dat">
          ...
          <textarea :disabled="currentVer!=lastVersion" class="ta8" placeholder="Descripcion" id="txtDescripcion" v-model="userStoryForm.fields.description"></textarea>
          <br /><br />
          <h5>Resultado Esperado del {{userStory.usType.title}}</h5>
          <textarea :disabled="currentVer!=lastVersion" class="ta8" placeholder="Resultado Espeado del Caso" id="txtResultadoHU" v-model="userStoryForm.fields.expectedResult"></textarea>
        ...
        <span slot="pre">
          <div style="min-height: 300px;">
            <pre-condition v-on:update="similarUserStories()" :inactive="currentVer!=lastVersion" :codeFormat.sync="userStory.usType.codeFormat" :preconditions.sync="userStoryForm.preConditions" :values.sync="values"></pre-condition>
            <hr />
            <div v-if="loadSimil & userStoriesSimil.length>0">
              <h5>Historias de Usuario Similares...</h5>
              <user-stories :data="userStoriesSimil" v-on:select="getUserStory()" :idUS.sync="idUS" v-on:found="sendToWorkpace()"></user-stories>
            </div>
            <div v-if="!loadSimil && userStoriesSimil.length==0">
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.isUserStoryExist) {
        ...
        this.dateFormat = this.formatDays(this.userStoryForm.us.versions[lastIndex].date);
        this.idHTML = this.userStoryForm.us.id;
        for (var v in this.userStoryForm.us.versions) {
          var ver = this.userStoryForm.us.versions[v];
          var reference = `Creada por ${ver.author}, ${this.formatDays(ver.date)}`
          this.versions.push({
            version: ver.version,
            ref: reference
          })
    ...
    methods: {
      ...
      code() {
        /*
        var code = ""
        this.userStory.usType.codeFormat.forEach(id => {
          if (typeof this.values[id] === 'undefined' || this.values[id] === "") {
            code += "????" + "_";
          } else {
            code += this.values[id] + "_";
          }
        });
        ...
        if (!this.codeOK) {
          ...
          this.userStoryForm.usType.codeFormat.forEach(id => {
            ...
            if (Object.keys(this.values).indexOf(id) > -1) {
              if (!(typeof this.values[id] === 'undefined' || this.values[id] === null || this.values[id] === "")) {
